...
contract SafeERC20 is ERC165
{
    ...
    {
        require(_to != address(this), "ERC20: Do not allow sending tokens to self.");
        require(_to != creator, "ERC20: Do not allow sending tokens to Converter via `transfer` func.");
        require(_to != address(0), "ERC20: Do not allow sending tokens to wrong address.");
        balances[msg.sender] = balances[msg.sender] - _value;
        balances[_to] = balances[_to] + _value;
        //emit Transfer(msg.sender, _to, _value, _data);
        emit Transfer(msg.sender, _to, _value);
    ...
    function name() public view returns (string memory)   { return IERC20(wrapper_for).name(); }
    function symbol() public view returns (string memory) { return IERC20(wrapper_for).symbol(); }
    function decimals() public view returns (uint8)       { return IERC20(wrapper_for).decimals(); }
    function origin() public view returns (address)                { return wrapper_for; }
...
contract GlobalTokenConverter is IERC223Recipient
{
    ...
    mapping (address => SafeERC20) public erc20Wrappers;   // A list of token wrappers. First one is ERC-20 origin, second one is ERC-223 version.
    ...
    mapping (address => uint256) public erc223Supply; // Token => how much was deposited.
    ...
    function getERC20OriginFor(address _erc223WrappedToken) public view returns (address)
    {
        return erc20Origins[_erc223WrappedToken];
    }

    function getERC223OriginFor(address _erc20WrappedToken) public view returns (address)
    {
        return erc223Origins[_erc20WrappedToken];
    }
    ...
    function tokenReceived(address _from, uint _value, bytes memory _data) public override returns (bytes4)
    {
        ...
        {
            // Convert ERC-223 Wrapper to ERC-20 token.
        ...
        if (erc20Origins[msg.sender] == address(0) && address(erc20Wrappers[msg.sender]) == address(0))
        {
            // Otherwise we are dealing with a ERC-223 token that needs to be
            // converted to ERC-20 wrapper.
            // Create wrapper.
    ...
    function createERC223Wrapper(address _erc20Token) public returns (address)
    {
        ...
        // Check if the provided pseudo-ERC-20 token is actually a ERC-223 token
        // which can be a wrapper for already existing ERC-20 token.
        require(getERC20OriginFor(_erc20Token) == address(0), "ERROR: Cannot convert ERC-223 to ERC-223.");
    ...
    // This function must create a ERC-20 wrapper for ERC-223 token
    // that is not wrapper for existing ERC-20 token in first place.
    function createERC20Wrapper(address _erc223Token) public returns (address)
    {
        // Check if the sent token is already a wrapper for existing ERC-20 token.
        require(getERC20OriginFor(_erc223Token) == address(0), "ERROR: Cannot convert ERC-223 to ERC-223.");

        // Check if wrapper already exists.
        require(address(erc20Wrappers[_erc223Token]) == address(0), "ERROR: Wrapper already exists.");

        SafeERC20 _newERC20Wrapper     = new SafeERC20(_erc223Token);
        erc20Wrappers[_erc223Token]              = _newERC20Wrapper;
        erc223Origins[address(_newERC20Wrapper)] = _erc223Token;
